...
class MyRoomsViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func observeMyRooms(){
      ...
      //    Database.database().reference().child("rooms").queryOrdered(byChild: "Owner").observeSingleEvent(of: .value, with: { (snapshot) in
        Database.database().reference().child("rooms").queryOrdered(byChild: "Owner").queryEqual(toValue:currentUser).observeSingleEvent(of: .value, with: { (snapshot) in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
            ...
            let destinationVC = segue.destination as! EnlargeMyRoomViewController
